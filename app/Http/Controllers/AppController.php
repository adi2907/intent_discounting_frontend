    ...
    public function mapIp(Request $request) {
        try{
            if($request->has('shop') && $request->filled('shop')) {
                ...
                $createArr = array_merge($updateArr, ['alme_token' => $token, 'session_id' => $request->session_id]);
                ...
                if($shop !== null && $shop->count() > 0) {
                    if(isset($shop->notificationSettings) && isset($shop->notificationSettings->sale_status)) {
                        if($shop->notificationSettings->sale_status == 1 || $shop->notificationSettings->sale_status === true) {
                            ...
                            return response()->json(['status' => true, 'message' => 'Turned off', 'blockSaleRequests' => true]);
                        } 
                    } else {
                        return response()->json(['status' => true, 'message' => 'Data not found', 'blockSaleRequests' => true]);
                    }
                } else {
                    return response()->json(['status' => true, 'message' => 'Shop Not Found', 'blockSaleRequests' => true]);
                }
            }
            return response()->json(['status' => true, 'message' => 'OK']);
        } catch(Exception $e) {
